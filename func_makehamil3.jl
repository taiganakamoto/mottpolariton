...
#dimension less current. using for making hamiltonian.
function make_current(nsite,nelec)
    ...
    for ith=1:nsite
        ...
        for ispin=1:2
            jspin = ispin
            J += -1im*make_cdc(ith,ispin,jth,jspin,nsite,nelec)
        end
...
function make_Current(nsite,nelec,η,N_c)
    ndim = combi(2*nsite,nelec)
    Ndim = ndim*(N_c+1)
    ...
    for ip=0:N_c
        ...
        jp = ip+1
        if jp <= N_c
            Current[(ip*ndim+1):(ip*ndim+ndim),(jp*ndim+1):(jp*ndim+ndim)] = (ζ*sqrt(jp))*J
        end
        jp = ip-1
        if jp >= 0
            Current[(ip*ndim+1):(ip*ndim+ndim),(jp*ndim+1):(jp*ndim+ndim)] = (ζ*sqrt(ip))*T
        end
    end
    return Current
end

function make_Current(nsite,nelec,ndown,η,N_c)
    ndim = combi(nsite,ndown)*combi(nsite,nelec-ndown)
    Ndim = ndim*(N_c+1)
    Current = spzeros(ComplexF64,Ndim,Ndim)
    ζ = η/sqrt(nelec)/(1+η^2)^(1/4)
